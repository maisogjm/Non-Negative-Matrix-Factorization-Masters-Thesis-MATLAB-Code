...
for k = kList
%disp(sprintf('k = %d\n',k))
system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/20Sim1.%s.%d.%s.chk',hostname,kIndex,runLabel)); % check file to see progress
    for sim = 1:numSims
...
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/Sim.%s.%d.%d.%s.FY_BIC.chk',hostname,kIndex,sim,runLabel)); % check file to see progress
        tic
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/GotThisFar3.chk')); % check file to see progress
        kHatvec      = FYandBICMultiresStrategy(A,4);
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/GotThisFar4.chk')); % check file to see progress
        elapsedTime  = toc;
        elapsedTime2 = elapsedTime*elapsedTime;
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/GotThisFar5.chk')); % check file to see progress
        ...
        k_BIC3       = kHatvec(4);
        k_rrssq      = kHatvec(5);
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/GotThisFar6.chk')); % check file to see progress
...
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/GotThisFar7.chk')); % check file to see progress
        sumSquaredElapsedTimes(kIndex,3) = sumSquaredElapsedTimes(kIndex,3) + elapsedTime2;
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/GotThisFar8.chk')); % check file to see progress
        sumSquaredElapsedTimes(kIndex,4) = sumSquaredElapsedTimes(kIndex,4) + elapsedTime2;
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/GotThisFar9.chk')); % check file to see progress
        sumSquaredElapsedTimes(kIndex,5) = sumSquaredElapsedTimes(kIndex,5) + elapsedTime2;
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/GotThisFar10.chk')); % check file to see progress
        sumSquaredElapsedTimes(kIndex,6) = sumSquaredElapsedTimes(kIndex,6) + elapsedTime2;

%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/AboutToCall_laplace_pca.chk')); % check file to see progress
...
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/AboutToCall_brunet.chk')); % check file to see progress
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/Sim.%s.%d.%d.%s.Brunet.chk',hostname,kIndex,sim,runLabel)); % check file to see progress
end
        tic
        [ k_brunet cophCoefVector alreadyTried ] = mitEstimateK_ntuNMF_Multires(A',subsamplingRate,cophThreshold,numRuns);
        elapsedTime                              = toc;
system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/20Sim1.Brunet.%d.%s.%d.%s.chk',k_brunet,hostname,kIndex,runLabel)); % check file to see progress
...
%        kHatVector                   = [ k_velicer k_FY k_BIC1 k_BIC2 k_BIC3 k_rrssq k_minka_laplace k_brunet ];
