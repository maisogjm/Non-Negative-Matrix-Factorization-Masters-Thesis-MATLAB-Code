function [ kHat cophCoefVector alreadyTried ] = mitEstimateK_mitNMF_Multires2( V , subsamplingRate , cophThreshFactor , numRuns )
%
...
% Note that we are using the MIT NMF code to compute the NMF, because the simulated data has exponentially
% rather than normally distributed noise.
%
% INPUTS:
...
consensus                            = nmfconsensus(V',kstart,kend,numRuns,0);
...
%plot(alreadyTriedIndices,cophCoefTmpList,'*-')
end
...
hiVal      = 20; % Don't try any values higher than 20
...
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/While.chk'));
while ( finished == 0 )
...
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/for_nc.chk'));
    for nc=indexList
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/if1.chk'));
        if ( nc > length(alreadyTried) )
            continue
        end

        % If this value for 'k' hasn't been tried yet, evaluate it.
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/if2.chk'));
...
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/if3.chk'));
    if ( stepSize <= 1 )
        finished = 1; 
...
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/if4.chk'));
        if ( loIndexOfIndex < 1 )
            loIndexOfIndex = 1;
        end
...
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/if5.chk'));
        ...
        % Update stepSize.
        stepSize = (hiVal-loVal)/subsamplingRate;
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/end1.chk'));
    end; % If stepSize
%[s,w]  = system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/end2.chk'));
end; % while finished

% Return current estimate of 'k', kHat.
%kHatRecord{k}.cophCoefVector      = cophCoefVector;
%kHatRecord{k}.cophCoefTmpList     = cophCoefTmpList;
%kHatRecord{k}.alreadyTriedIndices = alreadyTriedIndices;
%kHatRecord{k}.alreadyTried        = alreadyTried;
%kHatRecord{k}.kHat                = kHat;
return
