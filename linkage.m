...
if method == 'ce'  % square the Y so that it is easier to update.
   % distances between (merged) cluster centroids can be calculated from
   % the interpoint distances Y _only_ if Y is Euclidean distance.  If Y
   % is some other kind of dissimilarity, centroid should not be used.
   if ~iseuclidean(Y)
...
for s = 1:(n-1)
   ...
   if method == 'ce'
      ...
      % the tree is growing "downward" -- flag for a warning message
      if (s > 1) & (v < Z(s-1,3))
          monotonic = 0;
      end
   end
