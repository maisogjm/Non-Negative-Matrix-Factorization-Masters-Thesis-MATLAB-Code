...
system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/Sim.%s.%s.%s.%d.%d.chk',vers,hostname,run,trueK,rep)); % check file to see progress

    % Try Owen and Perry's BCV method.
    % 2-fold cross-validation over the 1000 rows.
    % 5-fold cross-validation over the 60 columns.
    tic
system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/Sim.genbicvgps.%s.%s.%s.%d.%d.chk',vers,hostname,run,trueK,rep)); % check file to see progress
    [ rowgps, colgps ] = genbicvgps(m, n, 2, 5);
system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/Sim.gengabcvsvd.%s.%s.%s.%d.%d.chk',vers,hostname,run,trueK,rep)); % check file to see progress
    gI                 = gengabcvsvd( A,  rowgps, colgps, kList, 0);
    elapsedTime        = toc;
system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/Sim.k_BCV.%s.%s.%s.%d.%d.chk',vers,hostname,run,trueK,rep)); % check file to see progress
    k_BCV(rep,:)       = gI(:);
system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/Sim.t_BCV.%s.%s.%s.%d.%d.chk',vers,hostname,run,trueK,rep)); % check file to see progress
...
%system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/InvokingFY_BIC_RRSSQ_KL.chk',hostname,run,trueK,rep));
    tic
    [ kVolume kBIC1 kBIC2 kBIC3 kRRSSQ ] = FY_BIC_RRSSQ_KL(A,kList);
    elapsed_time   = toc;

%system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/Volume.chk',hostname,run,trueK,rep));
    k_FY(rep,:)    = kVolume(:)';
%system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/BIC1.chk',hostname,run,trueK,rep));
    k_BIC1(rep,:)  = kBIC1(:)';
%system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/BIC2.chk',hostname,run,trueK,rep));
    k_BIC2(rep,:)  = kBIC2(:)';
%system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/BIC3.chk',hostname,run,trueK,rep));
    k_BIC3(rep,:)  = kBIC3(:)';
%system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/RRSSQ.chk',hostname,run,trueK,rep));
    k_rrssq(rep,:) = kRRSSQ(:)';
%system(sprintf('touch /data/jmm97/PrototypeBigSimulation3/progress/FinalSimulations/ALLDONE.chk',hostname,run,trueK,rep));
...
% Do this outside the 'rep' loop because it appears to do
% numRuns loops internally (accepts numRuns as an input parameter).
% Use Kullback-Liebler-based NMF computation method of Brunet et al.
